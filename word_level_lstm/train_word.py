...
parser.add_argument("--model_num", default=0, help="model to start or load from")
...
parser.add_argument("--model", default=0, help="Model name")
parser.add_argument("--embed_model", default='word2vec_padded.model', help="Model name for w2v embedding")
...
args.model_num=int(args.model_num); args.train_batch_size=int(args.train_batch_size); args.val_batch_size=int(args.val_batch_size)
...
if int(args.model_num) > 0:
    print(f"Loading model with epoch {str(args.model_num)}")
    model = load_model(args.checkpoints+"/"+args.model+"_"+str(args.model_num)+".h")
else:
    ...
    model = select_model(hidden_size=args.hidden_size, nb_features=w_table.size, n_layers=args.num_layers, model_name=args.model, maxlen=3, embed_size=args.embed_size)
...
for epoch in range(args.model_num,args.num_epochs+1):
    ...
    if not os.path.exists(args.checkpoints): os.makedirs(args.checkpoints)
    if epoch%20==0:
        save_path = args.checkpoints+"/"+args.model+"_"+str(epoch)+".h"
