...
from word_model import softmax_regression
...
def pad_tris(tris):
    ...
    for tri in tris:
        ...
        for tok in tri.split():
            padded_tok = '#'+tok
            padded_tok += '*' * (maxlen - len(padded_tok)-1) # Padded to maxlen.
            padded_tok += '>'
...
def batch_word_tris(tris, maxlen, wtable, batch_size=128, reverse=False):
    ...
    while(True):
        for i in range(batch_size):
            tri = next(tri_iterator)
            data_batch[i] = wtable.encode(tri, 3).reshape((3,w_table.size))
            #except: print(token)
        yield data_batch
...
if int(args.model_cnt) > 0:
    ...
    model = softmax_regression(nb_features=ctable.size)
