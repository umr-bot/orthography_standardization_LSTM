...
def batch_tris(tris, maxlen, wtable, batch_size=128, reverse=False):
    ...
    #data_batch = np.zeros((batch_size, 3, wtable.size),dtype=np.float32)
    data_batch = np.zeros((batch_size, 3),dtype=np.float32)
    while(True):
        for i in range(batch_size):
            tri = next(tri_iterator)
            for ind in range(3):
                if tri[ind] not in w_table.word2index: tri[ind] = '<unk>'
            data_batch[i] = np.array([w_table.word2index[tri[0]], w_table.word2index[tri[1]], w_table.word2index[tri[2]] ])
            #except: print(token)
        yield data_batch
...
if int(args.model_cnt) > 0:
    ...
    model = softmax_regression(nb_features=3) 
...
print("Number of val_steps:",val_steps)
 ...
 ###for epoch in range(args.model_cnt,args.num_epochs):
 ...
 ###        for tri_cnt in range(y_pred.shape[0]):
 ###            y_decoded.append(w_table.decode(y[tri_cnt].astype(int), calc_argmax=False))
 ###            y_pred_decoded.append(w_table.decode(y_pred[tri_cnt].astype(int), calc_argmax=False))
 ###            y_tar_decoded.append(w_table.decode(y_tar[tri_cnt].astype(int), calc_argmax=False))
 ###
 ###        print(y_decoded[tri_cnt][0:5][1],y_pred_decoded[tri_cnt][0:5][1],y_tar_decoded[tri_cnt][0:5][1])
 ###
