...
from models import lstm_model, lstm_model_n, lstm_model_n_dropout, bi_lstm_model_n, lstm_embed
import pickle
...
if int(args.model_num) > 0: 
    ...
    root_dir=args.checkpoints_dir+'/foldset_' + str(args.foldset_num)+'_hdd_'+str(args.hidden_size)
    #model = load_model( root_dir + "/"+"bi_lstm_3"+"_epoch_5"+".h5")
    model = load_model("checkpoints/eng/bi_lstm_3/foldset_3_hdd_1024/bi_lstm_3_epoch_5.h5")
...
for epoch in range(start,args.num_epochs):
    ...
    model_cur_file = '_'.join([args.model, 'epoch', 'n']) + '.h5'
    save_dir=args.checkpoints_dir + '/foldset_' + str(args.foldset_num)+'_hdd_'+args.hidden_size
    ...
    if (epoch+1) % 5 == 0:
        ...
        save_path = os.path.join(save_dir, model_cur_file)
        print('Saving full model to {:s}'.format(save_path))
        model.save(save_path)
